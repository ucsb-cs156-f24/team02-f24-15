...
describe("HelpRequestForm tests", () => {
  ...
  test("renders correctly when passing in a HelpRequest", async () => {
    ...
    expect(screen.getByTestId(/HelpRequestForm-requesterEmail/)).toHaveValue("student1@example.com");
    expect(screen.getByTestId(/HelpRequestForm-teamId/)).toHaveValue("team01");
    expect(screen.getByTestId(/HelpRequestForm-tableOrBreakoutRoom/)).toHaveValue("Table 1");
    expect(screen.getByTestId(/HelpRequestForm-explanation/)).toHaveValue("Need help with project setup");
  ...
  test("Correct Error messages on bad input", async () => {
    ...
    fireEvent.change(requesterEmailField, { target: { value: "email@domain" } });
  ...
  test("No Error messages on good input", async () => {
    ...
    expect(screen.queryByText(/Please enter a valid request time/)).not.toBeInTheDocument();
  });
