...
describe("UCSBOrganizationIndexPage tests", () => {
  ...
  test("renders three organizations correctly for regular user", async () => {
    ...
    const orgTranslationShort = screen.getByText("Gaucho Creative");
    expect(orgTranslationShort).toBeInTheDocument();

    const orgTranslation = screen.getByText("Gaucho Creative Consulting Club");
  ...
  test("what happens when you click delete, admin", async () => {
    ...
    axiosMock
      ...
      .reply(200, "UCSBOrganization with orgCode GCR deleted");
    ...
    await waitFor(() => {
      expect(mockToast).toHaveBeenCalledWith(
        "UCSBOrganization with orgCode GCR deleted",
      );
    });
    ...
    expect(axiosMock.history.delete[0].url).toBe("/api/ucsborganization");
    expect(axiosMock.history.delete[0].params).toEqual({ orgCode: "GCR" });
  });
});
